// ── PC scaling: fixed 1024×768, centred in window ─────────
(function () {
  const mediaPC = window.matchMedia('(pointer: fine) and (hover: hover)');

  function applyScale() {
    const ocean = document.getElementById('ocean');
    if (mediaPC.matches) {
      // PC: fixed 1024×768 canvas, scaled to fit window
      ocean.classList.add('pc-mode');
      ocean.style.height          = '';
      ocean.style.transform       = '';
      ocean.style.transformOrigin = '';
      const s = Math.min(window.innerWidth / 1024, window.innerHeight / 768);
      const x = (window.innerWidth  - 1024 * s) / 2;
      const y = (window.innerHeight -  768 * s) / 2;
  ...
  mediaPC.addEventListener('change', applyScale);
  window.addEventListener('resize', applyScale);
  // visualViewport fires its own resize on iOS when the keyboard / chrome changes
